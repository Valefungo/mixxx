...
EffectManifestPointer VocoderEffect::getManifest() {
    ...
    pManifest->setDescription(QObject::tr("Transform your voice to a robotic one"));
...
template<VocoderEffect::WaveShapes W>
void VocoderEffect::updateNoSub(CSAMPLE* _ab, const fpp_t _frames) {
    recalcPhase();
...
template<>
inline sample_t VocoderEffect::getSample<VocoderEffect::SineWave>(const float _sample) {
    return (sinSample(_sample));
}

template<>
inline sample_t VocoderEffect::getSample<VocoderEffect::TriangleWave>(const float _sample) {
    return (triangleSample(_sample));
}

template<>
inline sample_t VocoderEffect::getSample<VocoderEffect::SawWave>(const float _sample) {
    return (sawSample(_sample));
}

template<>
inline sample_t VocoderEffect::getSample<VocoderEffect::SquareWave>(const float _sample) {
    return (squareSample(_sample));
}

template<>
inline sample_t VocoderEffect::getSample<VocoderEffect::MoogSawWave>(const float _sample) {
    return (moogSawSample(_sample));
}

template<>
inline sample_t VocoderEffect::getSample<VocoderEffect::ExponentialWave>(const float _sample) {
    return (expSample(_sample));
}

template<>
inline sample_t VocoderEffect::getSample<VocoderEffect::WhiteNoise>(const float _sample) {
